...
import { Archive, Bell, Check, CheckSquare2, ChevronDown, Cloud, CloudOff, Download, FileText, Heart, Import, Laptop, ListFilter, LogOut, Menu, Moon, MoreHorizontal, Pin, Plus, Redo2, Search, Settings, Star, Sun, Trash2, User, WifiOff, X } from 'lucide-react';
...
export function NotesApp({ initialNotes, username, syncStatus, onNotesChange, onDeleteForever, onSignOut }: { initialNotes: Note[]; username: string; syncStatus: SyncStatus; onNotesChange: (notes: Note[]) => void; onDeleteForever: (id: string) => void; onSignOut: () => void }) {
  ...
  const firstNotesRender = React.useRef(true);
  ...
  React.useEffect(() => {
    ...
    if ('serviceWorker' in navigator) navigator.serviceWorker.register(publicAsset('sw.js'), { updateViaCache: 'none' }).catch(() => undefined);
  }, [announce]);

  React.useEffect(() => {
    if (firstNotesRender.current) { firstNotesRender.current = false; return; }
    onNotesChange(notes);
  }, [notes, onNotesChange]);
  ...
  const closeEditor = React.useCallback(async () => {
    ...
    if (isNoteEmpty(editing)) {
      if (notesRef.current.some((note) => note.id === editing.id)) setNotes((current) => current.filter((note) => note.id !== editing.id));
  ...
  const deleteForever = React.useCallback(async (note: Note) => {
    ...
    setNotes((current) => current.filter((item) => item.id !== note.id)); onDeleteForever(note.id); announce('Note deleted forever.');
  }, [announce, onDeleteForever]);
  ...
  return <main className="app-shell">
    <Sidebar section={section} counts={counts} colorFilter={colorFilter} online={online} username={username} syncStatus={syncStatus} onSection={selectSection} onColor={setColorFilter} onSettings={() => setSettingsOpen(true)} />
...
function SettingsDialog({ open, onOpenChange, dark, onTheme, online, username, installAvailable, onInstall, onExport, onImport, onSignOut }: { open: boolean; onOpenChange: (open: boolean) => void; dark: boolean; onTheme: (dark: boolean) => void; online: boolean; username: string; installAvailable: boolean; onInstall: () => void; onExport: () => void; onImport: () => void; onSignOut: () => void }) { return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="settings-dialog"><DialogHeader><DialogTitle>Settings</DialogTitle><DialogDescription>Make Color Notes feel at home on this device.</DialogDescription></DialogHeader><div className="settings-list"><Setting icon={<User />} title={`@${username}`} copy="Your notes are encrypted before they sync."><Button variant="outline" onClick={onSignOut}><LogOut />Sign out</Button></Setting><Setting icon={dark ? <Moon /> : <Sun />} title="Appearance" copy="Use a calm light or dark workspace."><Button variant="outline" onClick={() => onTheme(!dark)}>{dark ? 'Use light' : 'Use dark'}</Button></Setting><Setting icon={<Download />} title="Install app" copy="Open from your home screen and work offline."><Button variant="outline" onClick={onInstall}>{installAvailable ? 'Install' : 'How to'}</Button></Setting><Setting icon={<CloudOff />} title="Offline status" copy={online ? 'Online now. Your encrypted vault syncs automatically.' : 'Offline now. Changes stay encrypted here until you reconnect.'}><span className="ready-pill">Ready</span></Setting><Setting icon={<Download />} title="Backup" copy="Keep an extra copy that you control."><div className="setting-buttons"><Button variant="outline" size="sm" onClick={onExport}>Export</Button><Button variant="outline" size="sm" onClick={onImport}><Import />Import</Button></div></Setting></div></DialogContent></Dialog>; }
